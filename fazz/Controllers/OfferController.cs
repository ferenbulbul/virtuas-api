    ...
    public class OfferController : ControllerBase
    {
        ...
        public IActionResult GetMade(int? clinicId)
        {
            ...
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                {
                    var query =
                        "SELECT     app.id AS applicationId,    ca.title AS categoryTitle,    ca.credit AS cost,    u.name AS userName,    u.surname AS userSurname,    u.email AS userEmail,    u.phoneNumber as userPhone,    o.offerDate as offerDate FROM     applications app     JOIN users u ON u.id = app.userId     JOIN categories ca ON ca.id = app.categoryId        LEFT JOIN offers o ON o.applicationId = app.id AND o.clinicId = 72 WHERE     app.categoryId IN (        SELECT cl_ca.category_id         FROM clinic_categories cl_ca         WHERE cl_ca.clinic_id = 72    )    AND o.applicationId IS not NULL;";
